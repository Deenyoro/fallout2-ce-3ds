...
struct RecursiveLockGuard {
    RecursiveLock* lock;
    RecursiveLockGuard(RecursiveLock* l) : lock(l) { RecursiveLock_Lock(lock); }
    ~RecursiveLockGuard() { RecursiveLock_Unlock(lock); }
};
#endif
...
#ifdef __3DS__
        RecursiveLockGuard lockGuard(&soundBuffer->mutex);
#else
...
bool audioEngineInit()
{
    ...
    for (int i = 0; i < AUDIO_ENGINE_SOUND_BUFFERS; i++) {
        RecursiveLock_Init(&gAudioEngineSoundBuffers[i].mutex);
    }
#endif
