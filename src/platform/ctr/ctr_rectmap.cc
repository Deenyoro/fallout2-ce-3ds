...
rectMap_e getPreviousRectMap(int index)
{
    ...
    showMesageBox("getPreviousRectMap failed\n");
    return DISPLAY_FULL;
}
        ...
        float dst_x, float dst_y, float dst_w, float dst_h)
{
    ...
    if (rectMaps[rectmap][numRectsInMap[rectmap]] == NULL) {
        showMesageBox("addRectMap failed\n");
        exit(EXIT_FAILURE);
    }
...
void rectmap_init()
{
    if (rectMaps == NULL) {
        ...
        if (rectMaps == NULL || numRectsInMap == NULL) {
            showMesageBox("rectmap_init failed\n");
            exit(EXIT_FAILURE);
        }
    }
}
...
void ctr_rectmap_init()
{
    ...
    addRectMap(DISPLAY_ENDGAME,           0,   0,   0, 640, 480,   0,   0, 320, 240);
    addRectMap(DISPLAY_DYNAMIC,           0,   0,   0, 640, 480,   0,   0, 320, 240);
    addRectMap(DISPLAY_DEAD,              0,   0,   0, 640, 480,   0,   0, 320, 240);
}
